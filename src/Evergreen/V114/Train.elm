module Evergreen.V114.Train exposing (..)

import Duration
import Effect.Time
import Evergreen.V114.Color
import Evergreen.V114.Coord
import Evergreen.V114.Id
import Evergreen.V114.Tile
import Evergreen.V114.Units
import Quantity


type alias PreviousPath =
    { position : Evergreen.V114.Coord.Coord Evergreen.V114.Units.WorldUnit
    , path : Evergreen.V114.Tile.RailPath
    , reversed : Bool
    }


type IsStuckOrDerailed
    = IsStuck Effect.Time.Posix
    | IsDerailed Effect.Time.Posix (Evergreen.V114.Id.Id Evergreen.V114.Id.TrainId)
    | IsNotStuckOrDerailed


type Status
    = WaitingAtHome
    | TeleportingHome Effect.Time.Posix
    | Travelling
    | StoppedAtPostOffice
        { time : Effect.Time.Posix
        , userId : Evergreen.V114.Id.Id Evergreen.V114.Id.UserId
        }


type Train
    = Train
        { position : Evergreen.V114.Coord.Coord Evergreen.V114.Units.WorldUnit
        , path : Evergreen.V114.Tile.RailPath
        , previousPaths : List PreviousPath
        , t : Float
        , speed : Quantity.Quantity Float (Quantity.Rate Evergreen.V114.Units.TileLocalUnit Duration.Seconds)
        , home : Evergreen.V114.Coord.Coord Evergreen.V114.Units.WorldUnit
        , homePath : Evergreen.V114.Tile.RailPath
        , isStuckOrDerailed : IsStuckOrDerailed
        , status : Status
        , owner : Evergreen.V114.Id.Id Evergreen.V114.Id.UserId
        , color : Evergreen.V114.Color.Color
        }


type FieldChanged a
    = FieldChanged a


type TrainDiff
    = NewTrain Train
    | TrainChanged
        { position :
            FieldChanged
                { position : Evergreen.V114.Coord.Coord Evergreen.V114.Units.WorldUnit
                , path : Evergreen.V114.Tile.RailPath
                , previousPaths : List PreviousPath
                , t : Float
                , speed : Quantity.Quantity Float (Quantity.Rate Evergreen.V114.Units.TileLocalUnit Duration.Seconds)
                }
        , isStuckOrDerailed : FieldChanged IsStuckOrDerailed
        , status : FieldChanged Status
        }
