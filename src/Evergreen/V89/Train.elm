module Evergreen.V89.Train exposing (..)

import Duration
import Effect.Time
import Evergreen.V89.Color
import Evergreen.V89.Coord
import Evergreen.V89.Id
import Evergreen.V89.Tile
import Evergreen.V89.Units
import Quantity


type alias PreviousPath =
    { position : Evergreen.V89.Coord.Coord Evergreen.V89.Units.WorldUnit
    , path : Evergreen.V89.Tile.RailPath
    , reversed : Bool
    }


type IsStuckOrDerailed
    = IsStuck Effect.Time.Posix
    | IsDerailed Effect.Time.Posix (Evergreen.V89.Id.Id Evergreen.V89.Id.TrainId)
    | IsNotStuckOrDerailed


type Status
    = WaitingAtHome
    | TeleportingHome Effect.Time.Posix
    | Travelling
    | StoppedAtPostOffice
        { time : Effect.Time.Posix
        , userId : Evergreen.V89.Id.Id Evergreen.V89.Id.UserId
        }


type Train
    = Train
        { position : Evergreen.V89.Coord.Coord Evergreen.V89.Units.WorldUnit
        , path : Evergreen.V89.Tile.RailPath
        , previousPaths : List PreviousPath
        , t : Float
        , speed : Quantity.Quantity Float (Quantity.Rate Evergreen.V89.Units.TileLocalUnit Duration.Seconds)
        , home : Evergreen.V89.Coord.Coord Evergreen.V89.Units.WorldUnit
        , homePath : Evergreen.V89.Tile.RailPath
        , isStuckOrDerailed : IsStuckOrDerailed
        , status : Status
        , owner : Evergreen.V89.Id.Id Evergreen.V89.Id.UserId
        , color : Evergreen.V89.Color.Color
        }


type FieldChanged a
    = FieldChanged a
    | Unchanged


type TrainDiff
    = NewTrain Train
    | TrainChanged
        { position :
            FieldChanged
                { position : Evergreen.V89.Coord.Coord Evergreen.V89.Units.WorldUnit
                , path : Evergreen.V89.Tile.RailPath
                , previousPaths : List PreviousPath
                , t : Float
                , speed : Quantity.Quantity Float (Quantity.Rate Evergreen.V89.Units.TileLocalUnit Duration.Seconds)
                }
        , isStuckOrDerailed : FieldChanged IsStuckOrDerailed
        , status : FieldChanged Status
        }
